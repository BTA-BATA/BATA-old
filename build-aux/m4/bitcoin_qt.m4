...
dnl        PKG_CHECK_MODULES([X11XCB], [x11-xcb], [QT_LIBS="$X11XCB_LIBS $QT_LIBS"])
        _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QXcbIntegrationPlugin)],[-lqxcb -lxcb-static])
        AC_DEFINE(QT_QPA_PLATFORM_XCB, 1, [Define this symbol if the qt platform is xcb])
      elif test x$TARGET_OS = xdarwin; then
dnl        if test x$use_pkgconfig = xyes; then
dnl          PKG_CHECK_MODULES([QTPRINT], [Qt5PrintSupport], [QT_LIBS="$QTPRINT_LIBS $QT_LIBS"])
dnl              if ${PKG_CONFIG} --exists "Qt5Core < 5.4" 2>/dev/null; then
dnl              QT_LIBS="$QT_LIBS -L$qt_plugin_path/accessible"
dnl             _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(AccessibleFactory)], [-lqtaccessiblewidgets])
dnl          fi
dnl        fi
    ...
    if test x$TARGET_OS = xwindows; then
      AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
dnl      if test x$qt_plugin_path != x; then
dnl        QT_LIBS="$QT_LIBS -L$qt_plugin_path/accessible"
dnl        QT_LIBS="$QT_LIBS -L$qt_plugin_path/codecs"
dnl      fi
