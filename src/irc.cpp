...
// Copyright (c) 2011-2012 BATA Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BATATEST0\r");
            Send(hSocket, "WHO #BATATEST0\r");
        } else {
            // randomly join #BATA00-#BATA99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BATAcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BATAcoin%02d\r", channel_number).c_str());
        }
