...
uint256 hashGenesisBlock("0xb4bee36fd54a6176fd832f462641415c142d50e4b378f71c041870c2b1186bc8");
...
{
    // BATA: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BATA: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0x34, 0xc3, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BATAMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BATAMiner(CWallet *pwallet)
{
    printf("BATAMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BATAMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BATAMiner, pwallet));
}
